...
//! lookup -> incoming REFERENCES edges -> usage-site JSON" wiring around it.
...
/// Paginates the incoming `REFERENCES` edges for `anchor_id` and resolves
/// each one to its referencing node. Split out from `handle` so tests can
