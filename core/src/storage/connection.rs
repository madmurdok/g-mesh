...
///
/// The schema declares `edges`/`declarations`/`vectors` as foreign keys onto
/// `nodes`, and every writer is built on those declarations *not* being
/// enforced. Cross-file edges are lazy by design: when a symbol another file
/// points at is deleted or renamed, the importer's edge is left dangling until
/// that importer is itself reindexed (see `graph::imports::link_diff`). And
/// the plugin reports any change to a symbol as a delete plus an upsert of
/// the same id, without re-sending the unchanged edges into it - so under
...
/// the refusal was swallowed (GM-292). Setting it here rather than trusting
/// either default is what keeps that from depending on how the dependency
/// happens to be built.
///
/// Because nothing cascades, a delete from `nodes` owes its dependents an
/// explicit delete of their own - `storage::write::apply_diff` and
/// `graph::imports`' placeholder drop both do this. `ON DELETE CASCADE` in
/// the DDL is still honoured where a connection turns enforcement on (several
/// unit tests do, to catch an edge pointed at a node that was never written);
/// it is not what production relies on.
///
/// The two other places that open an index do not come through here -
/// `cli::status::index_status` and `gc::last_used::read_from_project_dir`,
/// which open an existing file without `CREATE` - and deliberately leave the
/// pragma alone: they only read, and foreign-key enforcement only ever
/// affects writes.
...
mod tests {
    ...
    /// GM-293. The bundled SQLite this crate links is compiled with
    /// `SQLITE_DEFAULT_FOREIGN_KEYS=1`, so leaving the pragma alone means
    /// enforcement is *on* - the opposite of what the write path is built
    /// for. Asked of the connection itself rather than of the build, so the
    /// test keeps meaning something if the dependency or its defines change.
    #[test]
