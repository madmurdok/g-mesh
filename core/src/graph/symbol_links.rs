...
mod tests {
    ...
    /// The whole legacy path, end to end, in the TS plugin's own v1 wire shape:
    /// `WireNode` deserialization derives the target, `watcher::apply`'s
    /// `to_node_record` carries it into storage, and the watcher's own
    /// `link_diff` links through it - including a renaming re-export, whose
    /// published name and target key differ.
    #[test]
    fn a_legacy_v1_placeholder_links_end_to_end_through_the_wire_path() {
        use crate::protocol::types::{WireEdge, WireNode};
        use crate::watcher::apply::{to_edge_record, to_node_record};

        /// One v1 `WireNode` line, shaped like the plugin's `--bulk-index`
        /// output (`exported`, no `visibility`, no `target`).
        fn node(
            id: &str,
            kind: &str,
            name: &str,
            ...
            let line = format!(
                r#"{{"id":"{id}","kind":"{kind}","name":"{name}","qualifiedName":"{qualified}","filePath":"{file}","range":{{"start":{{"line":0,"col":0}},"end":{{"line":0,"col":1}}}},"signature":null,"exported":{exported},"docComment":null,"language":"typescript","nativeKind":"{native}","hasSyntaxErrors":false}}"#
            );
            to_node_record(serde_json::from_str::<WireNode>(&line).unwrap())
        }
        fn edge(id: &str, from: &str, to: &str) -> EdgeRecord {
            let line = format!(
                r#"{{"id":"{id}","fromId":"{from}","toId":"{to}","kind":"CALLS","source":"tree-sitter","resolved":false}}"#
            );
            to_edge_record(serde_json::from_str::<WireEdge>(&line).unwrap())
        }
        ...
        let files = [
            Diff {
                upsert_nodes: vec![
                    node("caller-run", "Function", "run", "run", "caller.ts", true, "function"),
                    node(
                        "p-change",
                        "Module",
                        "change",
                ...
                upsert_edges: vec![
                    edge("e-direct", "caller-run", "p-change"),
                    edge("e-barrel", "caller-run", "p-renamed"),
                ],
                ..Default::default()
            },
            Diff {
        ...
        for diff in &files {
            apply_diff(&mut conn, diff).unwrap();
            linked += link_diff(&mut conn, diff).unwrap().linked_edges;
        }

        let key: String = conn
            .query_row("SELECT key FROM placeholder_targets WHERE nodeId = 'r-renamed'", [], |row| row.get(0))
            .unwrap();
        assert_eq!(key, "change", "the legacy derivation targets the real name, not the published alias");
        assert_eq!(linked, 2);
        assert_eq!(edge_target(&conn, "e-direct"), ("target-change".to_string(), true));
        assert_eq!(edge_target(&conn, "e-barrel"), ("target-change".to_string(), true));
    }
