...
# `target/<profile>/`, not the installed one), but not because of a naming defect
...
# `${G_MESH_BIN_DIR}/g-mesh-plugin-rust` - a path into the *workspace's*
...
main() {
	...
	local marker='command = "${G_MESH_BIN_DIR}/g-mesh-plugin-rust"'
